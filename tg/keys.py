...
from c_telegram import dp, BOT_INFO, bot, snippet, translate, POOL, request_level, db_config, json_config
            ...
            try:
                ...
                if giveaways:
                    ...
                    for giveaway in giveaways:
                        give_txt += f"\n{snippet['link'].format(text=giveaway['name'], link=get_giveaway_link(giveaway['id']))}: {count} 🎟"
                        await append_checker(message.chat.id, giveaway['id'])
                    key_text += give_txt
                stop_button = InlineKeyboardMarkup().add(InlineKeyboardButton(text=translate[cache['lang']]['generate_key'][7], callback_data="main_menu"))
